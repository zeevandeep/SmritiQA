"""
Test a single edge creation with a specific node pair.
"""
import os
import json
import logging
...
def test_single_edge_pair(pair_index):
    """Test creation of a single edge type."""
    if pair_index < 0 or pair_index >= len(TEST_PAIRS):
        logger.error(f"Invalid pair index. Must be between 0 and {len(TEST_PAIRS)-1}")
        return
    
    pair = TEST_PAIRS[pair_index]
    
    logger.info(f"=== Testing Edge Pair {pair_index+1} ===\n")
...
if __name__ == "__main__":
    # Get pair index from command line, default to 0
    pair_index = 0
    if len(sys.argv) > 1:
        try:
            pair_index = int(sys.argv[1])
        except ValueError:
            logger.error("Pair index must be an integer")
            exit(1)
    
    result = test_single_edge_pair(pair_index)
    if result:
        logger.info("\n=== Test Complete ===")