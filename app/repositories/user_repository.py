...
def mark_tour_completed(db: Session, user_id: UUID) -> bool:
    """
    ...
    try:
        ...
        if profile:
            ...
            db.commit()
            logger.info(f"Marked tour as completed for user {user_id}")
            return True
        else:
            logger.error(f"No profile found for user {user_id}")
            return False
    except Exception as e:
        logger.error(f"Error marking tour as completed for user {user_id}: {e}")
        db.rollback()
        return False


def get_user_tour_status(db: Session, user_id: UUID) -> bool:
    """
    ...
    Returns:
        True if tour is completed, False if not completed or user not found.
    """
    try:
        profile = db.query(UserProfile).filter(UserProfile.user_id == user_id).first()
        if profile:
            return profile.tour_completed or False
        return False
    except Exception as e:
        logger.error(f"Error getting tour status for user {user_id}: {e}")
        return False