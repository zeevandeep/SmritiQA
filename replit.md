...
- **PWA Exit Prompt Implementation**: Added native mobile app-style "press back again to exit" functionality across all pages. Intercepts back button presses to show orange-themed toast message "Press back again to exit Smriti" with 2-second timer window. Uses History API manipulation and includes graceful fallback for non-PWA contexts. Enhances mobile app experience when installed on devices
