...
- **Tour System Universal Fix Complete**: Resolved critical issue where Google OAuth users weren't receiving the app tour. Fixed signup flow to include welcome=true parameter for email/password users, corrected Google OAuth popup success handlers in login.html and signup.html to redirect existing users with welcome=true parameter, and fixed "Skip for now" link in language selection to include welcome=true parameter. Also moved App Tour option from Settings page to "How to Use Smriti" page as the first prominent section for better discoverability. All users now get the tour regardless of signup method or language selection choice
